...
/**
 * Use Case para obtener chats de Cuarentena con estado de bloqueo.
 * 
 * ACTUALIZADO: Usa threadId para verificar bloqueos.
 */
class GetQuarantineChatsUseCase @Inject constructor(
    private val chatRepository: ChatRepository,
    private val blockedSenderRepository: BlockedSenderRepository
) {
    /**
     * Obtiene todos los chats de Cuarentena enriquecidos con estado de bloqueo.
     */
    operator fun invoke(): Flow<List<Chat>> {
        return combine(
            chatRepository.getQuarantineChats(),
